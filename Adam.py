...
# This file is used to run Adam
# To run this type "python Adam.py"
# You can pipe the output as follows
# "python Adam.py > outputs/imsaves/Adam/AdamLog.txt"

# The default parameters here are beta1 = 0.9,beta2=0.999


def ADAM(A_input, b_input, x_input, probDist, maxIters, errorType=0, utType="Reg", tpType="Reg", momentumMult=0.1):
...
if __name__ == "__main__":
    ...
    numStates = 25
    numFeatures = 10
    filesLocation = "Outputs/imSaves/Adam/"
    ...
    for iter in range(10):
        ...
        x_out, errors = ADAM(A, b, start_x, probDist, maxIters, errorType=errorType, utType="Reg", tpType="Reg")
        print("Errors for iteration: %d" % (iter + 1))
        for i in range(len(errors)):
            print("%d,%.2e" % (i + 1, errors[i]))

        simName = str(iter + 1) + "Long Stochastic CurvatureStep TP with Adam Momentum"
        chartTitle = "Errors for Long Stochastic CurvatureStep TP with Adam Momentum"
